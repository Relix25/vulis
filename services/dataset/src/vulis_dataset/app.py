...
Mirrors the project-api handler — duplicated here so each service is
self-contained. A future refactor could extract this into a shared
helper library.
...
Every error response is JSON of the form::

    {
      "error": {
        "code": "NOT_FOUND",
        "message": "Dataset ds_abc not found",
        "details": {...},
        "correlation_id": "..."
      }
    }
...
def _build_engine(settings: DatasetSettings) -> Engine:
    dsn = settings.postgres_dsn
    if dsn is None:
        raise RuntimeError(
            "VULIS_POSTGRES_DSN must be set (or VULIS_DATASET_POSTGRES_DSN as override)."
        )
    # SecretStr unwrap — SQLAlchemy wants a plain string.
    return create_engine(dsn.get_secret_value(), future=True, pool_pre_ping=True)


def _build_storage(settings: DatasetSettings) -> StorageBackend:
...
def create_app(settings: DatasetSettings | None = None) -> FastAPI:
    ...
    app = FastAPI(
        title="Vulis Dataset API",
        version="0.1.0",
        description=(
            "M1.4 — Versioned, content-addressed datasets with manifest "
            "publishing and async bulk import."
        ),
        docs_url="/docs",
