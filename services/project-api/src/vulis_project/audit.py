...
Every state-changing operation in the project-api writes a row to the
shared ``audit_events`` table (created by libs/schemas migration 0001).
The table is append-only — there is a Postgres trigger that rejects
UPDATE and DELETE (see 0001_initial.py).
"""
